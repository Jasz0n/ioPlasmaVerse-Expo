...
import { Address, ADDRESS_ZERO, defineChain, estimateGas, eth_blockNumber, eth_call, eth_estimateGas, eth_gasPrice, eth_getBalance, eth_getTransactionByHash, eth_getTransactionCount, eth_getTransactionReceipt, getRpcClient, Hex } from "thirdweb";
...
const [signMessage, setSignMessage] = useState<{ message: string; id: number; tabId: string } | null>(null);
const [signResolve, setSignResolve] = useState<(() => void) | null>(null);
const [signReject, setSignReject] = useState<(() => void) | null>(null);
      ...
      try {
        const customHandlers: {
          ...
          personal_sign: async ({ account, params }) => {
            const [message, address] = params as WalletConnectSignRequestPrams;
            ...
            // Show sign modal
            setSignMessage({ message: decodedMessage, id: 0, tabId: activeTabId }); // id not used in WC, but kept for consistency
            setShowSignModal(true);
            await new Promise<void>((resolve, reject) => {
              setSignResolve(() => resolve);
              setSignReject(() => () => reject({ code: 4001, message: "User rejected signing" }));
            });
  
            return await account.signMessage({ message: decodedMessage });
          },
          eth_sign: async ({ account, params }) => {
            const [address, message] = params as WalletConnectSignRequestPrams;
            if (address.toLowerCase() !== account.address.toLowerCase()) {
              throw { code: 4001, message: "Address mismatch" } as WalletConnectRequestError;
            }
            const decodedMessage = Buffer.from(message.slice(2), "hex").toString();
            ...
            // Show sign modal
            setSignMessage({ message: decodedMessage, id: 0, tabId: activeTabId });
            setShowSignModal(true);
            await new Promise<void>((resolve, reject) => {
              setSignResolve(() => resolve);
              setSignReject(() => () => reject({ code: 4001, message: "User rejected signing" }));
          ...
          eth_sendTransaction: async ({ account, chainId, params }) => {
            const [txRaw] = params as WalletConnectTransactionRequestParams;
            if (txRaw.from?.toLowerCase() !== account.address.toLowerCase()) {
              throw { code: 4001, message: "Address mismatch" } as WalletConnectRequestError;
            }
            ...
            let gas = txRaw.gas ? BigInt(txRaw.gas) : undefined;
  
            if (!gas) {
              try {
                gas = await eth_estimateGas(rpcRequest, {
                  from: txRaw.from,
                  to: txRaw.to,
                  data: txRaw.data || "0x",
                ...
                gas = txRaw.data?.startsWith("0x095ea7b3") ? BigInt(70000) : BigInt(200000); // Approve vs. default
              }
            }
            ...
            const tx = {
                              ...
                              to: txRaw.to,
                              data: txRaw.data,
                              value: txRaw.value || "0x0" as `0x${string}`,
                              gas: txRaw.gas || toHex(gas),
                              gasPrice: txRaw.gasPrice || toHex(price),
                              nonce: txRaw.nonce || toHex(nonce),
                            }
  
            // Decode transaction data (reuse your decodeTxData function)
            const decodedTx = await decodeTxData(tx.data || "0x00");
            ...
            const txDetails = {
              ...
              gas: tx.gas.toString(),
              gasPrice: formatEther(tx.gasPrice) + " ETH",
              gasCost: formatEther(gas * price) + " ETH",
              data: tx.data?.slice(0, 20) + "...",
              decoded: decodedTx || undefined,
              dappUrl: tabs.find(t => t.id === activeTabId)?.url,
            };
            ...
            await new Promise<void>((resolve, reject) => {
              ...
              setTxReject(() => () => reject({ code: 4001, message: "User rejected transaction" }));
            });
          ...
          wallet_switchEthereumChain: async ({ account, chainId, params }) => {
            const [switchParams] = params as WalletConnectSwitchEthereumChainRequestParams;
            const newChainId = parseInt(switchParams.chainId.slice(2), 16);
            
            // Validate chain
            const supportedChains = [1, 4689, 10];
            if (!supportedChains.includes(newChainId)) {
              throw { code: 4902, message: "Unsupported chain ID" } as WalletConnectRequestError;
            }
  
            // Show chain switch modal
            setChainSwitchRequest({ chainId: newChainId, id: 0, tabId: activeTabId });
            await new Promise<void>((resolve, reject) => {
              setChainSwitchResolve(() => resolve);
              setChainSwitchReject(() => () => reject({ code: 4001, message: "User rejected chain switch" }));
            });
  
            await wallet.switchChain(defineChain(newChainId));
            setChain(newChainId);
            return switchParams.chainId as Hex;
          },
          // Keep other handlers as-is (e.g., eth_signTypedData, wallet_addEthereumChain)
          eth_signTransaction: async ({ account, chainId, params }) => {
            ...
            if (!gas) {
              try {
                ...
                console.error("Gas estimation failed:", error);
                gas = txRaw.data?.startsWith("0x095ea7b3") ? BigInt(70000) : BigInt(200000);
              }
            }
            ...
            // Show transaction modal for signing
            ...
            const txDetails = {
              ...
              gas: tx.gas.toString(),
              gasPrice: formatEther(tx.gasPrice) + " ETH",
              gasCost: formatEther(tx.gas * tx.gasPrice) + " ETH",
              data: tx.data.slice(0, 20) + "...",
              decoded: decodedTx || undefined,
              dappUrl: tabs.find(t => t.id === activeTabId)?.url,
            };
          ...
          wallet_addEthereumChain: async ({ account, chainId, params }) => {
            const [chainInfo] = params as WalletConnectAddEthereumChainRequestParams;
            const newChainId = parseInt(chainInfo.chainId, 16);
  
            // Show chain add modal
            setChainAddRequest({
              chainId: newChainId,
            ...
            await new Promise<void>((resolve, reject) => {
              ...
              setChainAddReject(() => () => reject({ code: 4001, message: "User rejected chain add" }));
            });
        ...
        const wc = await createWalletConnectClient({
          ...
          onConnect: async (session) => {
            console.log("WalletConnect session connecting:", session);
            // Show connect modal for session approval
            setShowConnectModal(true);
            await new Promise<void>((resolve, reject) => {
              setConnectResolve(() => resolve);
              setConnectReject(() => () => reject(new Error("User rejected connection")));
            });
...
`;
  ...
  return (
    ...
    <View style={styles.modalOverlay}>
      <View style={styles.modalContainer}>
        <ThemedText style={styles.modalTitle}>Sign Message</ThemedText>
        <ThemedText>Message: {signMessage?.message}</ThemedText>
        <ThemedText>From: {tabs.find(t => t.id === signMessage?.tabId)?.url}</ThemedText>
        <View style={styles.modalButtons}>
          <TouchableOpacity
