...
import { fetchPath, getAmountsIn, getAmountsOut, getAvailableFeeTiers } from "@/hooks/getAmounts";
...
import SwapDetails from "../dex/swapdetails";
...
export default function Dex() {
  ...
  const tabs: { id: TabType; label: string; icon: IconName }[] = [
    { id: 'swapData', label: 'Info', icon: 'person-outline' },
    { id: 'chart', label: 'Chart', icon: 'settings-outline' },
    { id: 'tokenList', label: 'tokenList', icon: 'cash-outline' },
    ...
    const fetchAmountsOut = async () => {
      if (currentFrom === "A" && tokenA && tokenB) {
        try {
          ...
          setDex(Amounts.bestDex);
          setFee(Amounts.fee);
          setRoute(Amounts.route);
          setDexStep(Amounts.bestDex);
          setFactoryAddress(Amounts.factory)
          if (tokenA.contractAddress.toLowerCase() == "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee" ) {
  ...
  }, [amountIn, currentFrom, tokenA, tokenB, chainId]);
  ...
  if (!tokenA) return;
  const parsedAmount = Math.floor(parseFloat(amountIn) * 10 ** tokenA.decimals);

    const fetchAmountsOut = async () => {
      if (currentFrom === "A" &&tokenA && tokenB && parsedAmount > 1n) {
        try{
        const url = `https://www.ioplasmaverse.com/api/getCurrencyData/getAmount?tokenIn=${tokenA.contractAddress}&tokenOut=${tokenB.contractAddress}&amount=${parsedAmount}&chainId=${chainId}&currentForm=${currentFrom}`;
             ...
             const Amounts = data.serializedAmount;   
  ...
  useEffect(() => {
    ...
    const fetchAmountsIn = async () => {
      if (currentFrom === "B" && tokenA && tokenB) {
        const parsedAmount = Math.floor(parseFloat(amountOut) * 10 ** tokenB.decimals);
  ...
  return (
    <View>
          ...
          <View style={styles.swapInputsContainer}>
            {tokenBalances.length > 0 && tokenA && tokenB && (
              ...
              <TokenBridgeSelector
                          ...
                          onAddToken={(token: Token) => handleSelectToken(token, "native")}
                          label={ "From Token"}
                        />
                  <TokenBridgeSelector
                          tokens={tokenBalances}
                          ...
                          onAddToken={(token: Token) => handleSelectToken(token, "token")}
                          label={ "To Token"}
                        />
      ...
      <View>
              {tokenA && tokenB && tab === "chart" &&(
                <CoinGeckoChart  Token={tokenB} symbol={symbol} poolAddress={tokenB.topPools}/>
              )}        

                {tokenA && tokenB && tab === "swapData" &&(
                  <View>
                  <SwapDetails tokenA={tokenA} tokenB={tokenB} amountIn={amountIn} amountOut={amountOut} />
                <ContractEvents tokenA={tokenA} tokenB={tokenB} />
                  </View>
                )}
               {tokenA && tokenB && tab === "tokenList" &&(
