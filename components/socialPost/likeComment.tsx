...
const LikeCommentComponent: React.FC<LikeComponentProps> = ({ postId, commentId, userId }) => {
  ...
  useEffect(() => {
    const fetchLikeData = async () => {
      ...
      const response = await fetch(`https://www.ioplasmaverse.com/api/socialPost/getLikeComments/${commentId}/${account?.address || ADDRESS_ZERO}/${postId}`);
    ...
    fetchLikeData();
  }, [postId, commentId, account?.address]);

  // Toggle like status
  const handleLikeToggle = async () => {
    
    try {
      const response = await fetch(`https://www.ioplasmaverse.com/api/socialPost/comment/likeComment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: account?.address, comment_id: commentId }),
      });
      const result = await response.json();
  ...
  // Send notification when the comment is liked
  const sendNotification = async () => {
    try {
      const notificationData = {
        user_id: userId,
        type: "like",
        message: `@${account?.address} has liked your comment #${commentId}.`,
      };
  ...
  const handleLongPress = async () => {
    ...
    try {
      const response = await fetch(`https://www.ioplasmaverse.com/api/socialPost/comment/likedUsersComments/${commentId}/${postId}`);
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      {/* Modal to show list of users who liked the comment */}
      <Modal
