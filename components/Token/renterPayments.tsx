...
export default function PaymentsScreen() {
  ...
  const fetchPayments = async (userId: string, pageNum: number) => {
    setIsLoading(true);
    try {
      const response = await fetch(`https://www.ioplasmaverse.com/api/pay/getPayments/${userId.toLowerCase()}?page=${pageNum}`);
      if (!response.ok) {
      ...
      if (Array.isArray(data.notifications)) {
        setPayments((prev) => (pageNum === 1 ? data.notifications : [...prev, ...data.notifications]));
      } else {
        console.error('Expected notifications to be an array, got:', data.notifications);
  ...
  return (
    <View style={styles.container}>
      {payments.map((item) => (
        ...
        >
          <PaymentInfo id={item.payment_id} amount={item.amount} date={new Date(item.created_at).toLocaleDateString()} status={item.is_payed} transactionHash={item.transaction_hash} chainId={Number(item.chain_id)} token={item.token_address.toString()} />
